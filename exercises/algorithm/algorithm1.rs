...
impl<T: std::cmp::PartialOrd + Clone> LinkedList<T> {
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        ...
        while cur_a.is_some() && cur_b.is_some() {
            let val_a = unsafe { cur_a.unwrap().as_ref().val.clone() };
            let val_b = unsafe { cur_b.unwrap().as_ref().val.clone() };
            if val_a < val_b {
                merged_list.add(val_a.clone());
                cur_a = unsafe { cur_a.unwrap().as_ref().next };
            } else {
                merged_list.add(val_b.clone());
                cur_b = unsafe { cur_b.unwrap().as_ref().next };
            }
        }
        while cur_a.is_some() {
            let val_a = unsafe { cur_a.unwrap().as_ref().val.clone() };
